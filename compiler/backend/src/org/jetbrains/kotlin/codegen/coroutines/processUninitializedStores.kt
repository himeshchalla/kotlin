...
class UninitializedStoresProcessor(private val methodNode: MethodNode, private val forCoroutines: Boolean) {
    ...
    fun run() {
        ...
        for ((index, insn) in methodNode.instructions.toArray().withIndex()) {
            ...
            if (usageInfo.storePopInsns.isEmpty()) continue
    ...
    private class UninitializedNewValueUsageInfo(newInsn: TypeInsnNode, typeInternalName: String) {
        ...
        val storePopInsns = mutableSetOf<AbstractInsnNode>()
        val otherCopyInsns = mutableSetOf<AbstractInsnNode>()

        fun add(insn: AbstractInsnNode) {
            when {
                insn.opcode == Opcodes.DUP -> dupInsns.add(insn)
                insn.isStoreOperation() -> storePopInsns.add(insn)
                else -> otherCopyInsns.add(insn)
            }
        }

        val isUsed get() = dupInsns.isNotEmpty() || storePopInsns.isNotEmpty() || otherCopyInsns.isNotEmpty()
        val allUsages get() = dupInsns + storePopInsns + otherCopyInsns
    }
    ...
    private class UninitializedNewValueMarkerInterpreter : OptimizationBasicInterpreter() {
        ...
        fun analyzePopInstructions(methodNode: MethodNode, frames: Array<Frame<BasicValue>?>) {
            ...
            for (i in frames.indices) {
                ...
                val insn = insns[i]
                if (insn.opcode == Opcodes.POP) analyzePop(insn, frame)
            }
        }

        private fun analyzePop(insn: AbstractInsnNode, frame: Frame<BasicValue>) {
            val value = frame.top() ?: error("Stack underflow on POP")
            val uninitializedNewValue = value as? UninitializedNewValue ?: return
            uninitializedValueUsageInfo[uninitializedNewValue.newInsn]!!.storePopInsns.add(insn)
        }
